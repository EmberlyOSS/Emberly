...
export default async function HomePage() {
  ...
  const testimonials = await prisma.testimonial.findMany({ where: { approved: true }, orderBy: { createdAt: 'desc' }, take: 6, include: { user: { select: { id: true, name: true, urlId: true } } } })
    ...
    <HomeShell>
      <main>
        <section className="max-w-7xl mx-auto px-4">
          ...
          {/** fetch partners server-side and hide if none */}
          {/** @ts-expect-error Server Component uses Prisma */}
          {/** partners retrieved above */}
          ...
          {testimonials && testimonials.length > 0 && (
            <TestimonialsList testimonials={testimonials.map((t) => ({ id: t.id, content: t.content, rating: t.rating, user: t.user }))} />
          )}
