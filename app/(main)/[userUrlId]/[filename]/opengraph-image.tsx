...
export default async function Image({ params }: Props) {
  ...
  // Fetch file info and site config in parallel
  const [fileResult, config] = await Promise.all([
    findFileByUrlPath(userUrlId, filename, {
  ...
  const typeInfo: TypeInfo = (() => {
    ...
    if (classification.isMusic || classification.isAudio) return { icon: '🎵', label: 'Audio' }
    if (classification.isImage) return { icon: '🖼️', label: 'Image' }
    if (classification.isDocument && fileResult?.mimeType?.includes('pdf')) return { icon: '📄', label: 'PDF Document' }
      ...
      >
        ...
        >
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '14px' }}>
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 375 375" width="42" height="42">
              <path
            ...
            <span style={{ fontSize: '30px', fontWeight: 700, color: foregroundColor, letterSpacing: '-0.01em' }}>
              Emberly
            </span>
          </div>
          ...
          >
            ...
            >
              ...
              <span style={{ fontSize: '20px', color: primaryColor, fontWeight: 600 }}>{typeInfo.label}</span>
            </div>

            {/* File name — shrink font if long */}
            <div
              style={{
                fontSize: fileName.length > 50 ? '32px' : fileName.length > 35 ? '38px' : '46px',
                fontWeight: 700,
            ...
            {/* File size */}
            {fileSize && (
              <div style={{ fontSize: '24px', color: mutedColor, marginTop: '16px', fontWeight: 400 }}>
                {fileSize}
              </div>
            )}
          </div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
            ...
            <span style={{ fontSize: '20px', color: mutedColor }}>Shared via Emberly</span>
          </div>
        </div>
      </div>
    ),
