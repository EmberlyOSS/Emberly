...
import {
  auditPaymentFailed,
  auditRefundIssued,
} from '@/packages/lib/events/audit-helper'
...
async function settleAppliedCredits(
  ...
  await prisma.$transaction(async (tx) => {
    ...
    const availableCreditCents = Math.max(
      0,
      Math.round((user?.referralCredits || 0) * 100)
    )
    ...
    switch (event.type) {
      case 'checkout.session.completed': {
        ...
        if (session.mode === 'subscription' || session.subscription) {
          ...
          const product =
            (stripeSub.items.data[0].price.product as any)?.id || null
          ...
          if (user) {
            ...
            if (
              sessionMetadata.type === 'storage-bucket' &&
              ...
              await provisionUserStorageBucket(
                ...
                logger.error(
                  `[Webhook] Failed to provision storage bucket for user ${user.id}`,
                  err
                )
              })
            }
            ...
            logger.info(
              `[Webhook] Subscription created for user ${user.id}: ${subId}`
            )
          }
        } else {
          ...
          if (user) {
            ...
            if (creditsApplied > 0) {
              ...
              console.log(
                `[Webhook] Applied $${creditsApplied / 100} credit to user ${user.id} for purchase`
              )
            }
            ...
            if (purchaseType === 'extra_storage') {
              ...
              await prisma.user.update({
                where: { id: user.id },
                data: { storageQuotaMB: current + addMB },
              })
            ...
            if (purchaseType === 'custom_domain') {
              logger.info(
                `[Webhook] Added ${qty} domain slot(s) to user ${user.id}`
              )
            }
        ...
        if (subscriptionId) {
          ...
          if (user) {
            ...
            const resolvedProductId =
              dbProduct?.id || stripeProductId || 'unknown'
            ...
            const isStorageBucketSub = (dbProduct?.slug || '').startsWith(
              'storage-bucket'
            )
            if (!existingBucket && isStorageBucketSub) {
              const tierFromSlug = dbProduct?.slug?.replace(
                'storage-bucket-',
                ''
              )
              const recoveryRegion =
                (stripeSub.metadata?.location as string | undefined) || ''
              const recoveryTier =
                (stripeSub.metadata?.tier as string | undefined) || tierFromSlug
              ...
              await provisionUserStorageBucket(
                ...
                logger.error(
                  `[Webhook] Invoice recovery provisioning failed for user ${user.id}`,
                  err
                )
              })
            }
            ...
            logger.info(
              `[Webhook] Invoice paid for user ${user.id}: $${amountPaid / 100}`
            )
          }
        }
        break
      ...
      case 'customer.subscription.updated': {
        ...
        if (user) {
          ...
          if (!resolvedProductId) {
            logger.warn(
              `[Webhook] Unable to map product for subscription ${stripeSubId}`
            )
            break
          }
      ...
      case 'customer.subscription.deleted': {
        ...
        if (existing) {
          ...
          await deprovisionUserStorageBucket(stripeSubId, existing.user).catch(
            (err) => {
              logger.error(
                `[Webhook] Failed to deprovision storage bucket for sub ${stripeSubId}`,
                err
              )
            }
          )
...
/**
 * Provisions an Object Storage bucket for a user after they subscribe.
 * Finds the admin-provisioned ObjectStoragePool for the chosen region,
...
/**
 * Deprovisions the provider bucket tied to a cancelled Stripe subscription.
 * Deletes the bucket from its provider, removes the StorageBucket record and
...
async function deprovisionUserStorageBucket(
  ...
  const storageBucket = await prisma.storageBucket.findFirst({
    ...
    include: { objectStoragePool: true },
  })

  if (!storageBucket) return

  const { objectStoragePool, poolBucketName } = storageBucket

  if (objectStoragePool && poolBucketName) {
    try {
      switch (objectStoragePool.provider) {
        case 'vultr':
          await deleteObjectStorageBucket(
            objectStoragePool.externalId,
            poolBucketName
          )
          break
        case 'linode': {
          const meta = objectStoragePool.metadata as {
            linodeClusterId?: string
          } | null
          if (meta?.linodeClusterId)
            await deleteLinodeBucket(meta.linodeClusterId, poolBucketName)
          break
        }
        case 'ovhcloud': {
          const meta = objectStoragePool.metadata as {
      ...
      logger.info(
        `[Deprovision] Deleted bucket '${poolBucketName}' from ${objectStoragePool.provider} pool ${objectStoragePool.id}`
      )
    } catch (err) {
      logger.error(
        `[Deprovision] Failed to delete bucket '${poolBucketName}' from provider`,
        err as Error
      )
