...
export interface UploadMetadata {
  ...
  s3UploadId: string
  domain?: string | null
  /** ID of the StorageBucket record the multipart upload was initiated against. Null = global config bucket. */
...
export const uploadCache = {
  ...
  async save(uploadId: string, metadata: UploadMetadata): Promise<boolean> {
    ...
    try {
      ...
      logger.error('Failed to save upload metadata', error as Error, {
        uploadId,
      })
      return false
    }
  ...
  async get(uploadId: string): Promise<UploadMetadata | null> {
    ...
    try {
      ...
      logger.error('Failed to get upload metadata', error as Error, {
        uploadId,
      })
      return null
    }
  ...
  async update(
    ...
    try {
      ...
      logger.error('Failed to update upload metadata', error as Error, {
        uploadId,
      })
      return false
    }
  ...
  async delete(uploadId: string): Promise<boolean> {
    ...
    try {
      ...
      logger.error('Failed to delete upload metadata', error as Error, {
        uploadId,
      })
      return false
    }
