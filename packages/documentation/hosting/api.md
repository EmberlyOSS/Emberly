> Authentication uses NextAuth session cookies in the browser. API clients can pass `Authorization: Bearer <uploadToken>` for some upload flows (chunked upload finalize).
